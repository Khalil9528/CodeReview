...
public class AwesomePasswordChecker {
    ...
    /**
     *Changing the letters of the password that will be hided
     * @param password the password that will be coded after the end ef the program
     * @return
     */
    public int[] maskAff(String password) {
        ...
        int[] maskArray = new int[password.length()];
    ...
    public double getDistance(String password) {
        int[] maskArray = maskAff(password); //here we convert our password to the int array
        double minDistance = Double.MAX_VALUE; //initialise minDistance with the maximum of double
    ...
    public static String ComputeMD5(String input) {
        ...
        int numBlocks = ((messageLenBytes + 8) >>> 6) + 1;  // Calculate the number of 512-bit blocks
        int totalLen = numBlocks << 6;                     // Total padded length in bytes
        byte[] paddingBytes = new byte[totalLen - messageLenBytes];
        paddingBytes[0] = (byte) 0x80;                     // Add the `1` bit

        long messageLenBits = (long) messageLenBytes << 3; // Original message length in bits
        ByteBuffer lengthBuffer = ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putLong(messageLenBits);
        byte[] lengthBytes = lengthBuffer.array();         // Convert length to bytes

        byte[] paddedMessage = new byte[totalLen];         // Combine everything into padded message
